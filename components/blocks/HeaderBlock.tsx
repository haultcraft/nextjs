	...
	return (
		<>
			<header className='max-w-screen-xl mx-auto px-6 lg:px-8 xl:px-4 py-4 lg:py-6 flex justify-between'>
				...
				<label htmlFor='nav-mobile' className='lg:hidden'><FontAwesomeIcon icon={['fas', 'bars']}
				...
				<input
					...
					className='hidden' />
				<nav
					className='flex absolute top-[-200%] text-lg flex-col left-0 px-6 py-2 bg-white z-10 w-screen nav-collapsable transition-all ease-in duration-200 lg:static  lg:flex-row lg:gap-4 lg:justify-end lg:p-0 lg:m-0 lg:w-full lg:items-center'>
					{
						links.map(item => (
							<Link key={item.url} href={item.url}>
								<a
									className={(router.pathname === item.url ? 'font-semibold' : 'text-gray-500') + ' flex gap-2 items-center hover:text-gray-700 mt-2 lg:mt-0 mt-2 lg:mt-0'}>
									{item.label}
								</a>
							</Link>
						))
